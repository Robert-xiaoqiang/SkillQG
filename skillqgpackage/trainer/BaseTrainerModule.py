...
'''
    This class `BaseTrainerModule` can not be instantiated because the decoding strategy is not implemented here.
    It must be mixed-in by TrainerModuleCLMMixin or TrainerModuleSeq2SeqMixin.
    By the way, it can not be regarded as a mix-in class due to its quantities of parameters/attributes.
'''
class BaseTrainerModule(pl.LightningModule, TrainerModuleEvalMixin):#, metaclass = abc.ABCMeta):
